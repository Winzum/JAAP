...
func _on_save_pressed() -> void:
	show_file_dialog(FileDialog.FILE_MODE_SAVE_FILE, 'json', Callable(self, 'save_file'))

func _on_load_pressed() -> void:
	show_file_dialog(FileDialog.FILE_MODE_OPEN_FILE, 'json', Callable(self, 'load_file'))

func _on_export_epub_pressed() -> void:
	show_file_dialog(FileDialog.FILE_MODE_SAVE_FILE, 'epub', Callable(self, 'export_epub'))
...
func show_file_dialog(mode: int, file_type: String, callable: Callable):
...
func export_epub(file_path: String) -> void:
	...
	if not (origin_node == "" or connections.size() == 0):
		...
		epub_node.ExportEpub(file_path)
